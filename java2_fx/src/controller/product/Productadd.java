...
public class Productadd implements Initializable {
    ...
    private String pimage = null; // 메소드 밖에어 선언하는 이유 : imgadd메소드와 add 메소드에서 사용하기 위해

    @FXML
    void imgadd(ActionEvent event) { // 이미지등록 버튼을 클릭했을때
    	// 1. 파일 선택 클래스
    	...
    	try {
        	...
        	FileInputStream inputStream = new FileInputStream(file); // file : fileChosser 에서 선택된 파일 객체
        	// 2. 파일 출력 스트림
        		// 1. 새로운 경로 설정 												/ 빠졌어요
        	...
        	int size;
        	while( (size = inputStream.read(bytes)) >0 ) { // 읽어온 바이트가 0보다 작으면 반복문 종료 [ 읽어올바이트가 없을때까지]
