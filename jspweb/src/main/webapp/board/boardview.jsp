...
<body>
 ...
 <!----------------------------------- 게시물 삭제/수정/목록 버튼 구역 ------------------------------------------------->		
		...
		%>
			<div class="col-md-2">
				<a href="bdelete?bno=<%=board.getBno()%>"> <button class="form-control">삭제</button> </a> <!--작성자와 로그인된 id가 동일하면 보이는 버튼  -->
			</div>	
			<div class="col-md-2">
				<a href="bupdate.jsp?bno=<%=board.getBno()%>"> <button class="form-control">수정</button> </a> <!--작성자와 로그인된 id가 동일하면 보이는 버튼  -->
			</div>
 ...
 <!----------------------------------- 댓글 구역 쓰기 -------------------------------------------------------------------->		
		...
		<div class="row"> <!-- row : 가로배치 -->
			<div class="col-md-10">
				<textarea name="rcontent" id="rcontent" class="form-control" rows=3 ></textarea>
			</div>
			...
			<%ArrayList<Reply> rereplylist = BoardDao.getBoardDao().rereplylist( bno , reply.getRno() );
				...
				<tr>
					...
					<% if( mid != null && mid.equals( rereply.getMid() ) ){ %>
						<button class="btn replybtn" onclick="reupdateview(<%=rereply.getRno()%>,'<%=rereply.getRcontent()%>',<%=rereply.getBno()%>)"> 수정 </button>
